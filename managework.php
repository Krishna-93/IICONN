...
{   $table_id = "translation_Job_id";
    ...
    $query = "SELECT * FROM `translation_job_table` WHERE `translator_interpreter_id` = $translator_interpreter_id
...
}else if(!empty($keywordsearch))
{
  ...
  if($_POST['filter']=="due_date" || $_POST['filter']=="date_of_submission")
  {
    ...
    WHERE `".$_POST['filter']."` LIKE '%".$newformat."%'
	  AND `translator_interpreter_id` = $translator_interpreter_id ORDER BY `translation_Job_id` "
...
}elseif (!empty($pn))
{
  ...
  $query = "SELECT * FROM `translation_job_table`
  WHERE `translator_interpreter_id` = $translator_interpreter_id
  ORDER BY `translation_Job_id` desc $limit";
  ...
  $query = "SELECT * FROM `translation_job_table`
  WHERE `translator_interpreter_id` = $translator_interpreter_id
  ORDER BY `translation_Job_id` desc ".pagination_managework($table_id,$table,$page_rows,$pn, $translator_interpreter_id);
...
<head>
  ...
  <title>IICONN - Manage Work</title>
<style type="text/css">
    #side-bar {
      width:100%;
      float: left;
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                  <div class="col-md-6">
                    ...
                    <div class="input-group">
                      <span class="input-group-btn">
                        <form action="managework.php" method="post" class="navbar-form" role="search">
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td>
                            ...
                            <?php echo "<a href=\"view_translationjoboverview2.php?id=".$value['translation_Job_id']."&id_update=".$value['translation_Job_id']."\"
