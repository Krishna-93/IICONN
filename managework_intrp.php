...
{   $table_id = "interpretation_job_id";
    ...
    $page_rows = 5;
    $last = numb_pages_managework($table_id,$table,$page_rows, $translator_interpreter_id);
    //$query = "SELECT * FROM `translation_job_table` ".pagination($table_id,$table,$page_rows,$pn);
    $query = "SELECT * FROM `interpretation_job_table` WHERE `translator_interpreter_id` = $translator_interpreter_id
    order by `".$order_parameters .
     "` ". $sort_parameters. " ".pagination_managework($table_id,$table,$page_rows,$pn,$translator_interpreter_id);
}else if(!empty($keywordsearch))
{
  ...
  if($_POST['filter']=="date of submission" || $_POST['filter']=="date_of_interpretation" || $_POST['filter']=="time_of_interpretation")
  {
    if($_POST['filter']=="date of submission" || $_POST['filter']=="date_of_interpretation")
    {
      ...
      AND `translator_interpreter_id` = $translator_interpreter_id
      ORDER BY `interpretation_job_id` "
      .pagination_managework($table_id,$table,$page_rows,$pn,$translator_interpreter_id);
    }
    else
    {
...
{
  $row_count = "Select COUNT(`interpretation_job_id`) FROM `interpretation_job_table` WHERE `translator_interpreter_id` = $translator_interpreter_id";
  ...
  $query = "SELECT * FROM `interpretation_job_table`
  WHERE `translator_interpreter_id` = $translator_interpreter_id
  ORDER BY `interpretation_job_id` desc ".pagination_managework($table_id,$table,$page_rows,$pn,$translator_interpreter_id);
...
<head>
  ...
  <title>IICONN - Manage Work Interpretation</title>
<style type="text/css">
    #side-bar {
      width:100%;
      float: left;
                  ...
                  <div class="col-md-6">
                    <div class="input-group">
                                ...
                                <select name="page_switch" id="page_switch" class="form-control">
                                     <option value="managework_intrp.php">Interpretation Table</option>
                                    <option value="managework.php">Translation Table</option>
                                </select>
                            </span>
                          </div><!-- /input-group -->
                        </div>
                          ...
                          <div class="input-group">
                            <span class="input-group-btn">
                              <form action="managework_intrp.php" method="post" class="navbar-form" role="search">
                        ...
                        <div class="pcw-wrap">
                            ...
                            <tbody>
                              ...
                              <tr>
                                ...
                                <td>
                                  <?php echo "<a href=\"view_interpretationjoboverview2.php?id=".$value['interpretation_job_id']."&id_update=".$value['interpretation_job_id']."\"
