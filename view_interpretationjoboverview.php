...
`Grant_name`, `lang_to`, `special_instructions`, `invoice`  From `interpretation_job_table` Where `interpretation_job_id` = $job_id ";
 ...
 <?php
  if(isset($_POST['submit']))
  {
    ...
    $dateOfInterpretation = mysqli_real_escape_string($connection,trim($_POST['dateOfInterpretation']));
    $timeOfInterpretation = mysqli_real_escape_string($connection,trim($_POST['timeOfInterpretation']));
    $lengthOfInterpretation = mysqli_real_escape_string($connection,trim($_POST['lengthOfInterpretation']));
    $grant = mysqli_real_escape_string($connection,trim($_POST['grant']));
    $InterpretationTo = mysqli_real_escape_string($connection,trim($_POST['lang_from']));
    ...
    SET `job_status` = '$jobStatus',
     `first_name` = '$firstname', `last_name` = '$lastname', `email` = '$emailAddress',
      ...
      `gender` = '$gender', `date_of_interpretation` = '$dateOfInterpretation',
      `time_of_interpretation` = '$timeOfInterpretation',  `length_of_interpretation` = '$lengthOfInterpretation',
      `Grant_name` = '$grant', `lang_to` = '$InterpretationTo',
    ...
    if ($result2) {
      ...
      alert('Interpretation job updated successfully.'); </script>";
 ...
 <?php
	...
	{	$translator_interpreter_id = $_SESSION['user']['translator_interpreter_id'];
		$query = "UPDATE `interpretation_job_table` a,
		`translator_interpreter_table` b
		SET a.translator_interpreter_id = $translator_interpreter_id, a.job_status = 'Ongoing'
		WHERE a.interpretation_job_id = $job_id AND b.translator_interpreter_id = $translator_interpreter_id";
    ...
    if ($result = mysqli_query($connection, $query))
    {
      ...
      window.location = \" managework_intrp.php \";
    </script>
        ";
  ...
  try {
   ...
   ->setSubject('Successfully selected new Interpretation Job')
  ...
  //
	...
	$query_updated = "SELECT `job_title`, `job_status`, `translator_interpreter_id`, `first_name`, `last_name`, `email`,
 `address`, `city`, `state`, `zip_code`, `gender`, `date of submission`,
  `date_of_interpretation`, `time_of_interpretation`, `length_of_interpretation`,
`Grant_name`, `lang_to`, `special_instructions`, `invoice`  From `interpretation_job_table` Where `interpretation_job_id` = $job_id ";

  $result = mysqli_query($connection, $query_updated);
  $Row = (mysqli_fetch_assoc($result));
  $jobTitle = $Row['job_title'];
  $jobStatus = $Row['job_status'];
  $jobAssignedTo = $Row['translator_interpreter_id'];
        ...
        <div id="page-content-wrapper" style="padding-left:130px;">
         ...
         <div class="panel-body">
           <p></p>
                            ...
                            <form method="post" action="view_interpretationjoboverview.php" accept-charset='UTF-8' enctype="multipart/form-data" >
         <div class="container-fluid">
           <div class="form-group">
             <p> Below you can view the information of the current job. You can select this job by clicking select job. </p>
<br />
             <div class="form-group">
  ...
  <div class="form-group">
    ...
    <div class="form-group">
           <label for="ft_author">Special Instructions:</label>
                         <textarea class="form-control" name="ft_message" id="textarea" placeholder="Special instructions" cols="25" rows="10" maxlength="300" disabled><?php echo stripslashes($specialInstructions); ?></textarea>
             ...
             <button type="button" class="btn btn-primary" onclick="window.location.href = 'view_interpretationjob.php'">Back</button>
             <!-- <button type="submit" class="btn btn-primary"  formaction="view_interpretationjoboverview.php?id=<?php echo $job_id?>" name="submit">Update existing job</button> -->
              <button type="submit" class="btn btn-primary"  formaction="view_interpretationjoboverview.php?id=<?php echo $job_id?>" name="selectjob">Select job</button>
            </div></form>
</div>
 </div>
</div>
