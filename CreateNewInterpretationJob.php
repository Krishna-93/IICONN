...
//    `Grant_name`, `Job_status`)
 ...
 VALUES ('".mysqli_real_escape_string($connection,trim($_POST['ClientsFirstName']))."', '".
  ...
  if($result){
    ...
    window.location = \" interp_admindashboard.php \";

  </script>
      ";

      $query_last_row = "SELECT * FROM `interpretation_job_table` ORDER BY `interpretation_job_id` DESC LIMIT 1";
      $result_last_row = mysqli_query($connection, $query_last_row);
      $row_last = mysqli_fetch_assoc($result_last_row);

      $query_recipients = "SELECT `email`,
      CONCAT(`first_name`,' ', `last_name`)
      AS name FROM `translator_interpreter_table`";

      $result_recipients = mysqli_query($connection, $query_recipients);
    ...
    try {
      ...
      ->setSubject('New interpretation job scheduled for '.
      $row_last['date_of_interpretation'] . ' @ '.
      $row_last['time_of_interpretation'] . ', interested? '
      )
      ...
      ' <head></head>' .
      ' <body>' .
      '<p>A new job has been created.  For more details please go to your job pool.</p>' .
      '<p>Sincerely,<br />' .
      'Claudia Connor,<br />' .
      'Administrator</p>' .
     ' </body>' .
     ...
     'text/html'
      ...
      while($recipients = mysqli_fetch_assoc($result_recipients))
      {
        $email_recipients += array($recipients['email'] => $recipients['name']);
      }

      foreach ($email_recipients as $key => $value)
      {
 ...
 <div id="page-content-wrapper" style="padding-left:130px;">
  ...
  <div class="panel-body">
    <p></p>
                     ...
                     <form method="post" action="CreateNewInterpretationJob.php" accept-charset='UTF-8' >
  <div class="container-fluid">
    <div class="form-group">
                      ...
                      <button type="reset" class="btn btn-primary" onclick="window.location.href = 'CreateNewInterpretationJob.php'">Reset</button>
